...
// --- Types ---
type Screen = 'splash' | 'onboarding' | 'auth' | 'home' | 'explore' | 'tourism' | 'restaurants' | 'hotels' | 'jobs' | 'emergency' | 'municipality' | 'profile' | 'services_grid' | 'design_system' | 'blood_donation' | 'transport' | 'courier' | 'electricity' | 'technician' | 'land' | 'education' | 'shopping' | 'business';
        ...
        {activeTab === 'donor' ? (
          <>
            <button 
              ...
              className="w-full h-14 bg-red-50 dark:bg-red-900/10 border-2 border-dashed border-red-200 dark:border-red-900/30 rounded-2xl flex items-center justify-center gap-3 text-red-600 font-display font-bold active:scale-95 transition-transform mb-2"
            >
              {showDonorForm ? <X size={20} /> : <Plus size={20} />}
              {showDonorForm ? "Close Form" : "Add Your Donor Profile"}
            </button>
            ...
            <AnimatePresence>
              {showDonorForm && (
                ...
                >
                  <div className="glass-card dark:bg-slate-900 p-6 rounded-[32px] mb-6 shadow-xl border-2 border-red-100 dark:border-red-900/20">
                    <h3 className="text-sm font-display font-black text-slate-800 dark:text-white mb-4 uppercase tracking-wider">Donor Registration</h3>
                    <div className="space-y-4">
                      <div className="flex gap-4">
                        <div className="flex-1 space-y-1.5">
                          <label className="text-[10px] font-black text-slate-400 uppercase tracking-widest ml-1">Blood Group</label>
                          <select className="w-full h-12 bg-slate-50 dark:bg-slate-800 rounded-xl px-4 text-xs font-bold outline-none border border-slate-100 dark:border-slate-700 appearance-none">
                            {bloodGroups.filter(g => g !== 'All').map(g => <option key={g} value={g}>{g}</option>)}
                          </select>
                        </div>
                        <div className="flex-1 space-y-1.5">
                          <label className="text-[10px] font-black text-slate-400 uppercase tracking-widest ml-1">Last Donated</label>
                          <input type="date" className="w-full h-12 bg-slate-50 dark:bg-slate-800 rounded-xl px-4 text-xs font-bold outline-none border border-slate-100 dark:border-slate-700" />
                        </div>
                      </div>
                      <div className="space-y-1.5">
                        <label className="text-[10px] font-black text-slate-400 uppercase tracking-widest ml-1">Preferred Location</label>
                        <input type="text" placeholder="e.g. Mongla Port Area" className="w-full h-12 bg-slate-50 dark:bg-slate-800 rounded-xl px-4 text-xs font-bold outline-none border border-slate-100 dark:border-slate-700" />
                      </div>
                      <button className="w-full h-12 bg-red-600 text-white rounded-xl font-display font-black text-[11px] uppercase tracking-wider shadow-lg shadow-red-200 dark:shadow-none">Register as Donor</button>
                    </div>
                  </div>
                </motion.div>
              )}
          ...
          <>
            <button 
              ...
              className="w-full h-14 bg-primary/5 border-2 border-dashed border-primary/20 rounded-2xl flex items-center justify-center gap-3 text-primary font-display font-bold active:scale-95 transition-transform mb-2"
            >
              {showRequestForm ? <X size={20} /> : <Plus size={20} />}
              {showRequestForm ? "Close Form" : "Post Blood Request"}
            </button>
            ...
            <AnimatePresence>
              {showRequestForm && (
                <motion.div 
                  ...
                  className="overflow-hidden"
                >
                  <div className="glass-card dark:bg-slate-900 p-6 rounded-[32px] mb-6 shadow-xl border-2 border-primary/10">
                    <h3 className="text-sm font-display font-black text-slate-800 dark:text-white mb-4 uppercase tracking-wider">New Blood Request</h3>
                    <div className="space-y-4">
                      <div className="flex gap-4">
                        <div className="flex-1 space-y-1.5">
                          <label className="text-[10px] font-black text-slate-400 uppercase tracking-widest ml-1">Required Group</label>
                          <select className="w-full h-12 bg-slate-50 dark:bg-slate-800 rounded-xl px-4 text-xs font-bold outline-none border border-slate-100 dark:border-slate-700 appearance-none">
                            {bloodGroups.filter(g => g !== 'All').map(g => <option key={g} value={g}>{g}</option>)}
                          </select>
                        </div>
                      ...
                      <div className="space-y-1.5">
                        ...
                        <input type="text" placeholder="e.g. Mongla Upazila Health Complex" className="w-full h-12 bg-slate-50 dark:bg-slate-800 rounded-xl px-4 text-xs font-bold outline-none border border-slate-100 dark:border-slate-700" />
                      </div>
                      <button className="w-full h-12 bg-primary text-white rounded-xl font-display font-black text-[11px] uppercase tracking-wider shadow-lg shadow-primary/20">Submit Request</button>
                    </div>
                  </div>
                </motion.div>
              )}
...
function LandServicesScreen({ onBack }: { onBack: () => void }) {
  return (
    <div className="flex flex-col pb-32">
      <ScreenHeader title="Land Services" subtitle="Digital land records & tools" onBack={onBack} action={<FileText className="text-secondary" size={28} />} />
      <div className="px-6 space-y-4 mt-4">
         {[
            { title: "E-Porcha / Khatian", desc: "Find digital land records", icon: FileText, bg: "bg-emerald-50", color: "text-emerald-600" },
            { title: "Mutation Application", desc: "Apply for land mutation", icon: Edit2, bg: "bg-blue-50", color: "text-blue-600" },
            { title: "Land Tax (Khajna)", desc: "Pay your land development tax", icon: ElectricityIcon, bg: "bg-amber-50", color: "text-amber-600" },
            { title: "Maps & Mouza", desc: "View digitized mouza maps", icon: MapIcon, bg: "bg-purple-50", color: "text-purple-600" },
         ].map((s, i) => (
           <div key={i} className="glass-card dark:bg-slate-900 rounded-[32px] p-6 shadow-xl border-none flex items-center gap-5 active:scale-95 transition-transform group">
             <div className={`w-14 h-14 ${s.bg} dark:bg-slate-800 rounded-2xl flex items-center justify-center group-hover:bg-primary transition-colors`}>
               <s.icon size={26} className={`${s.color} group-hover:text-white transition-colors`} />
             </div>
             <div className="flex-1">
               <h3 className="font-display font-black text-slate-800 dark:text-slate-100">{s.title}</h3>
...
function ElectricityScreen({ onBack }: { onBack: () => void }) {
  return (
    <div className="flex flex-col pb-32">
      <ScreenHeader title="Electricity" subtitle="Power & Utility Services" onBack={onBack} action={<ElectricityIcon className="text-amber-500" size={28} />} />
      <div className="px-6 space-y-4 mt-4">
        <div className="bg-primary p-8 rounded-[40px] text-white shadow-xl shadow-primary/20 relative overflow-hidden">
           ...
           <p className="text-[10px] font-black uppercase tracking-[3px] mb-2 opacity-70">Hotline 24/7</p>
           <h2 className="text-4xl font-display font-black mb-6">16114</h2>
           <button className="w-full h-14 bg-white/20 backdrop-blur-md rounded-2xl font-bold flex items-center justify-center gap-2 border border-white/30">
             <Phone size={20} /> Call Complaint Center
           </button>
        </div>

        <h3 className="text-xs font-black text-slate-400 uppercase tracking-widest ml-2">Announcements</h3>
        <div className="space-y-4">
           {[
             { title: "Load Shedding Update", desc: "No planned load shedding for today in Ward 01-09.", icon: CheckCircle2, color: "text-emerald-500", bg: "bg-emerald-50" },
             { title: "Grid Maintenance", desc: "System maintenance scheduled for next Friday at Port Area.", icon: Info, color: "text-blue-500", bg: "bg-blue-50" },
           ].map((a, i) => (
             <div key={i} className="glass-card dark:bg-slate-900 p-5 rounded-3xl border-none shadow-md flex items-start gap-4">
                <div className={`w-12 h-12 ${a.bg} dark:bg-slate-800 rounded-xl flex items-center justify-center shrink-0`}>
                  <a.icon size={24} className={a.color} />
                </div>
                <div>
                   <h4 className="font-display font-bold text-slate-800 dark:text-slate-100">{a.title}</h4>
                   <p className="text-xs text-slate-500 font-medium leading-relaxed mt-1">{a.desc}</p>
                </div>
             </div>
           ))}
        </div>
...
function EducationScreen({ onBack }: { onBack: () => void }) {
  ...
  const institutions = [
    { name: "Mongla Government High School", address: "Ward 04, Mongla", phone: "017XX-XXXXXX", website: "mghs.edu.bd" },
    { name: "Mongla College", address: "Main Road, Mongla", phone: "018XX-XXXXXX", website: "monglacollege.edu.bd" },
  ];

  return (
    <div className="flex flex-col pb-32">
      <ScreenHeader title="Education" subtitle="Learning & academic centers" onBack={onBack} action={<GraduationCap className="text-primary" size={28} />} />
      <div className="px-6 flex gap-3 overflow-x-auto no-scrollbar mb-4 mt-2">
        {['School', 'College', 'University', 'Madrasa', 'Coaching'].map(c => (
          <button 
            key={c}
            onClick={() => setActiveCat(c)}
            className={`h-11 px-5 rounded-xl text-[10px] font-black uppercase tracking-wider transition-all whitespace-nowrap ${activeCat === c ? 'bg-primary text-white shadow-lg' : 'glass-card dark:bg-slate-900 text-slate-400'}`}
      ...
      <div className="px-6 space-y-4">
        {institutions.map((inst, i) => (
          <div key={i} className="glass-card dark:bg-slate-900 rounded-[32px] p-6 shadow-xl border-none">
            <div className="flex gap-4 mb-6">
              <div className="w-20 h-20 bg-slate-100 dark:bg-slate-800 rounded-2xl flex items-center justify-center text-slate-400 overflow-hidden">
                <img src={`https://images.unsplash.com/photo-1523050853064-59f6f32e74c6?auto=format&fit=crop&q=80&w=200`} className="w-full h-full object-cover" />
              </div>
              <div className="flex-1">
                <h3 className="text-lg font-display font-black text-slate-800 dark:text-white leading-tight">{inst.name}</h3>
                <p className="text-xs font-sans font-medium text-slate-500 mt-1">{inst.address}</p>
                <div className="mt-3">
                   <Badge variant="primary">{activeCat}</Badge>
                </div>
              </div>
            </div>
            <div className="flex gap-3">
              <ActionButton icon={Phone} label="Contact" />
              <ActionButton icon={Globe} label="Website" variant="ghost" />
            </div>
          </div>
        ))}
      </div>
    ...
    <div className="flex flex-col pb-32">
      ...
      <div className="px-6">
        <div className="flex justify-between items-center mb-6">
          ...
          <button className="text-primary text-[10px] font-black uppercase flex items-center gap-1 group">
...
function BusinessScreen({ onBack }: { onBack: () => void }) {
  const cats = [
    { id: 'hosp', label: 'Hospital', icon: Hospital, color: 'text-rose-500', bg: 'bg-rose-50' },
    { id: 'clinic', label: 'Clinic', icon: Stethoscope, color: 'text-blue-500', bg: 'bg-blue-50' },
    { id: 'doctor', label: 'Doctor', icon: User, color: 'text-indigo-500', bg: 'bg-indigo-50' },
    { id: 'pharmacy', label: 'Pharmacy', icon: Star, color: 'text-emerald-500', bg: 'bg-emerald-50' },
    { id: 'bank', label: 'Bank', icon: Landmark, color: 'text-amber-500', bg: 'bg-amber-50' },
    { id: 'atm', label: 'ATM', icon: Smartphone, color: 'text-slate-600', bg: 'bg-slate-100' },
  ];

  return (
    <div className="flex flex-col pb-32">
      <ScreenHeader title="Directory" subtitle="Smart local business finder" onBack={onBack} action={<SearchCode className="text-primary" size={28} />} />
      
      <div className="px-6 mb-4 mt-2">
        <div className="bg-white dark:bg-slate-900 rounded-2xl h-14 flex items-center px-4 gap-3 shadow-md border border-slate-100 dark:border-slate-800 mb-4">
          <Search size={20} className="text-slate-400" />
          <input type="text" placeholder="Search hospitals, banks, doctors..." className="bg-transparent flex-1 outline-none text-sm font-sans font-bold" />
        </div>

        <div className="grid grid-cols-3 gap-6">
          {cats.map(c => (
            <button key={c.id} className="flex flex-col items-center gap-2 group">
              <div className={`w-16 h-16 ${c.bg} dark:bg-slate-900 rounded-[24px] flex items-center justify-center shadow-sm border border-slate-100 dark:border-slate-800 group-hover:bg-primary transition-all`}>
                <c.icon size={26} className={`${c.color} group-hover:text-white transition-colors`} />
              </div>
              <span className="text-[10px] font-black text-slate-800 dark:text-slate-400 uppercase tracking-tight">{c.label}</span>
            </button>
          ))}
        </div>
      </div>
      ...
      <div className="px-6 mt-4">
         <h3 className="text-sm font-black text-slate-400 uppercase tracking-widest mb-4 ml-2">Verified Recommendations</h3>
         <div className="space-y-4">
            {[
              { name: "City Trust Bank", type: "Bank & ATM", reviews: 45, rating: 4.8, img: "https://images.unsplash.com/photo-1541339907198-e08759df9a0a?fit=crop&w=400" },
              { name: "Care Vision Clinic", type: "Medical Center", reviews: 82, rating: 4.9, img: "https://images.unsplash.com/photo-1519494026892-80bbd2d6fd0d?fit=crop&w=400" },
            ].map((p, i) => (
              <div key={i} className="glass-card dark:bg-slate-900 rounded-[32px] p-4 flex gap-4 border-none shadow-lg">
                 <img src={p.img} className="w-24 h-24 rounded-2xl object-cover" />
                 <div className="flex-1 py-1 flex flex-col justify-between">
                    <div>
                      <div className="flex justify-between items-start">
                        <h4 className="font-display font-black text-slate-800 dark:text-white">{p.name}</h4>
                        <div className="flex items-center gap-1">
                          <Star size={12} className="fill-accent text-accent" />
                          <span className="text-[10px] font-black">{p.rating}</span>
                        </div>
                      </div>
                      <p className="text-[10px] font-black text-slate-400 uppercase mt-1">{p.type}</p>
                    </div>
                    <div className="flex gap-2">
                       <button className="h-8 px-4 bg-primary text-white rounded-lg text-[9px] font-black uppercase tracking-wider">Contact</button>
                       <button className="h-8 px-4 glass-card dark:bg-slate-800 text-slate-500 rounded-lg text-[9px] font-black uppercase tracking-wider">Map</button>
                    </div>
                 </div>
              </div>
            ))}
...
export default function App() {
  ...
  const renderScreen = () => {
    switch (currentScreen) {
      ...
      case 'onboarding': return <OnboardingScreen onComplete={() => setCurrentScreen('auth')} onNext={() => setOnboardingStep(s => s + 1)} step={onboardingStep} />;
      case 'auth': return <AuthScreen onLogin={() => { setIsLoggedIn(true); setCurrentScreen('home'); }} />;
      ...
      case 'hotels': return <HotelsScreen onBack={() => setCurrentScreen('home')} />;
      case 'jobs': return <JobBoardScreen onBack={() => setCurrentScreen('home')} />;
      ...
      case 'electricity': return <ElectricityScreen onBack={() => setCurrentScreen('home')} />;
      case 'technician': return <TechnicianScreen onBack={() => setCurrentScreen('home')} />;
      case 'land': return <LandServicesScreen onBack={() => setCurrentScreen('home')} />;
      case 'education': return <EducationScreen onBack={() => setCurrentScreen('home')} />;
      case 'shopping': return <ShoppingScreen onBack={() => setCurrentScreen('home')} />;
      case 'business': return <BusinessScreen onBack={() => setCurrentScreen('home')} />;
      case 'municipality': return <MunicipalityScreen onBack={() => setCurrentScreen('home')} />;
      case 'profile': return <ProfileScreen onLogout={() => { setIsLoggedIn(false); setCurrentScreen('auth'); }} toggleTheme={toggleTheme} isDarkMode={isDarkMode} onDesignSystem={() => setCurrentScreen('design_system')} />;
  ...
  return (
    <div className={`min-h-screen w-full flex justify-center bg-slate-background selection:bg-primary/30 ${isDarkMode ? 'dark' : ''}`}>
      ...
      <div className={`relative w-full max-w-[440px] min-h-[90vh] my-4 rounded-4xl overflow-hidden ${isDarkMode ? 'bg-slate-950 text-white' : 'bg-background text-text'} shadow-[0_40px_100px_-20px_rgba(15,23,42,0.3)] border-[8px] border-slate-ink`}>
        ...
        {/* Global Bottom Navigation (Visible after Auth) */}
        {isLoggedIn && currentScreen !== 'splash' && currentScreen !== 'onboarding' && currentScreen !== 'auth' && (
...
function SplashScreen() {
  return (
    <div className="flex flex-col items-center justify-center min-h-screen bg-gradient-to-br from-primary to-secondary relative">
      <div className="absolute inset-0 overflow-hidden opacity-20">
        <div className="absolute top-[-10%] left-[-10%] w-80 h-80 bg-white rounded-full blur-3xl animate-pulse" />
        <div className="absolute bottom-[-10%] right-[-10%] w-80 h-80 bg-accent rounded-full blur-3xl animate-pulse delay-700" />
      </div>
      <motion.div 
    ...
    <div className="flex flex-col pb-32">
      ...
      <header className="px-6 pt-8 pb-6 bg-white dark:bg-slate-950 sticky top-0 z-40 flex items-center justify-between">
        <div className="flex items-center gap-3">
          ...
          <div>
            <p className="label-caps">Assalamu Alaikum</p>
            <h1 className="text-xl font-display font-black text-slate-900 dark:text-white">Abdul Ajij Noman👋</h1>
          </div>
        </div>
        <div className="flex gap-3">
      ...
      <section className="px-6 mb-4">
        <div className="relative overflow-hidden rounded-3xl h-48 group shadow-2xl">
          <img src="https://images.unsplash.com/photo-1540333032872-5b62b7785999?auto=format&fit=crop&q=80&w=800" className="w-full h-full object-cover group-hover:scale-105 transition-transform duration-700" alt="Hero" />
      ...
      <section className="px-6 mb-4">
        ...
        <div className="flex gap-4 overflow-x-auto pb-4 -mx-6 px-6 no-scrollbar">
          {[
            { id: 1, name: 'Shat Gombuj Masjid', img: 'https://images.unsplash.com/photo-1596422846543-75c6fc183f27?fit=crop&w=400&q=80', distance: '12 km' },
            { id: 2, name: 'Karamjal Tourism', img: 'https://images.unsplash.com/photo-1610484826967-09c5720778c7?fit=crop&w=400&q=80', distance: '5 km' },
            { id: 3, name: 'Mongla Town Hall', img: 'https://images.unsplash.com/photo-1574169208507-84376144848b?fit=crop&w=400&q=80', distance: '0.5 km' },
          ].map(p => (
...
function ProfileScreen({ onLogout, toggleTheme, isDarkMode, onDesignSystem }: { onLogout: () => void, toggleTheme: () => void, isDarkMode: boolean, onDesignSystem: () => void }) {
  return (
    <div className="flex flex-col p-6 pb-32">
      ...
      <div className="space-y-4">
        ...
        <button 
          onClick={onLogout}
          className="w-full bg-red-50 dark:bg-red-900/10 p-5 rounded-[24px] flex items-center justify-center gap-3 border-2 border-dashed border-red-200 dark:border-red-900/30 text-red-500 font-bold active:scale-[0.98] transition-transform mt-4"
        >
...
function HotelsScreen({ onBack }: { onBack: () => void }) {
  const hotels = [
    { name: "Sundarban Gateway Hotel", price: "৳3500/night", rating: 4.8, reviews: 156, img: "https://images.unsplash.com/photo-1566073771259-6a8506099945?fit=crop&w=400" },
    { name: "Mongla Port Guest House", price: "৳2200/night", rating: 4.5, reviews: 89, img: "https://images.unsplash.com/photo-1542314831-068cd1dbfeeb?fit=crop&w=400" },
    { name: "Pashur Tourist Lodge", price: "৳1500/night", rating: 4.2, reviews: 45, img: "https://images.unsplash.com/photo-1445019980597-93fa8acb246c?fit=crop&w=400" },
  ];

  return (
    <div className="flex flex-col pb-32">
       <ScreenHeader title="Hotels" subtitle="Premium stay in Mongla" onBack={onBack} action={<Building className="text-primary" size={28} />} />
       
       <div className="px-6 mt-4 space-y-4">
          <div className="flex gap-2 overflow-x-auto no-scrollbar">
             {['All', 'Premium', 'Eco Friendly', 'Budget', 'Nearby'].map(f => (
               <button key={f} className="px-5 py-2 glass-card rounded-full text-[10px] font-black uppercase whitespace-nowrap active:bg-primary active:text-white transition-colors">{f}</button>
             ))}
          </div>
          ...
          {hotels.map((hotel, i) => (
             <div key={i} className="glass-card dark:bg-slate-900 rounded-[32px] overflow-hidden shadow-xl border-none">
                <div className="h-52 relative">
                   ...
                   <div className="absolute bottom-4 left-4 bg-primary text-white text-[10px] font-black px-4 py-2 rounded-xl shadow-lg">
                      {hotel.price}
                   </div>
                </div>
                <div className="p-6">
                   <h3 className="text-xl font-display font-black text-slate-800 dark:text-white mb-2">{hotel.name}</h3>
                   <div className="flex items-center justify-between mb-6">
                      <div className="flex items-center gap-2 text-slate-400">
                         <MapPin size={12} className="text-primary" />
                         <span className="text-[10px] font-bold">MONGLA PORT AREA</span>
                      </div>
                      <span className="text-[10px] text-slate-400 font-bold">{hotel.reviews} REVIEWS</span>
                   </div>
                   <div className="flex gap-3">
                      <ActionButton icon={CalendarDays} label="Book Stay" />
                      <ActionButton icon={Phone} label="Call" variant="ghost" />
                   </div>
                </div>
             </div>
          ))}
...
function JobBoardScreen({ onBack }: { onBack: () => void }) {
  const [showPostForm, setShowPostForm] = useState(false);
  
  return (
    <div className="flex flex-col p-0 pb-32">
       <ScreenHeader title="Job Portal" subtitle="Find your next career move" onBack={onBack} action={<Briefcase className="text-primary" size={28} />} />

       <div className="px-6 mt-4">
          <div className="bg-amber-50 dark:bg-amber-900/10 border-2 border-amber-100 dark:border-amber-900/30 rounded-3xl p-5 mb-8">
             <div className="flex gap-3">
                <AlertTriangle className="text-amber-600 shrink-0" size={20} />
                <p className="text-[11px] font-sans font-bold text-amber-800 dark:text-amber-200 leading-relaxed">
                   প্রতারক হতে সাবধান! অনলাইনে কোনো অপরিচিত ব্যক্তি বা প্রতিষ্ঠানকে অর্থ লেনদেন করলে অথবা প্রতারণার শিকার হলে কর্তৃপক্ষ দায়ী থাকবেনা।
                </p>
             </div>
          </div>

          <div className="flex items-center justify-between mb-8">
             <h2 className="text-lg font-display font-black text-slate-900 dark:text-white">Active Jobs</h2>
             <button 
                onClick={() => setShowPostForm(!showPostForm)}
                className="bg-primary/10 text-primary text-[10px] font-black uppercase px-4 py-2 rounded-xl flex items-center gap-1.5"
             >
          ...
          {showPostForm && (
            <motion.div 
               ...
               animate={{ opacity: 1, scale: 1 }}
               className="glass-card dark:bg-slate-900 p-6 rounded-[32px] mb-8 shadow-xl border-2 border-primary/10"
             >
                <h3 className="text-sm font-display font-black text-slate-800 dark:text-white mb-4 uppercase tracking-wider">Create Job Post</h3>
                <div className="space-y-4">
                   <div className="space-y-1.5">
                      <label className="text-[10px] font-black text-slate-400 uppercase tracking-widest ml-1">Job Title</label>
                      <input type="text" placeholder="e.g. Graphic Designer" className="w-full h-12 bg-slate-50 dark:bg-slate-800 rounded-xl px-4 text-xs font-bold outline-none border border-slate-100 dark:border-slate-700" />
                   </div>
                   <div className="space-y-1.5">
                      <label className="text-[10px] font-black text-slate-400 uppercase tracking-widest ml-1">Salary Range</label>
                      <input type="text" placeholder="e.g. ৳20k - ৳30k" className="w-full h-12 bg-slate-50 dark:bg-slate-800 rounded-xl px-4 text-xs font-bold outline-none border border-slate-100 dark:border-slate-700" />
                   </div>
                   <button className="w-full h-12 bg-primary text-white rounded-xl font-display font-black text-[11px] uppercase tracking-wider">Publish Post</button>
                </div>
            </motion.div>
          )}
          ...
          <div className="space-y-4">
            {[
              { title: "Junior Web Developer", company: "Mongla Soft", salary: "৳25k - 35k", type: "Remote", color: "bg-blue-50 text-blue-600" },
              { title: "Shift Manager", company: "Portside Logistics", salary: "৳45k - 60k", type: "Full-time", color: "bg-emerald-50 text-emerald-600" },
              { title: "Tourist Guide", company: "Sundarban Travalia", salary: "৳15k - 20k", type: "Part-time", color: "bg-amber-50 text-amber-600" },
              { title: "Security Guard", company: "City Protection", salary: "৳12k - 15k", type: "Full-time", color: "bg-purple-50 text-purple-600" },
            ].map((job, i) => (
              <div key={i} className="glass-card dark:bg-slate-900 p-6 rounded-[28px] border-none shadow-md">
                <div className="flex justify-between items-start mb-4">
                ...
                <h3 className="text-lg font-display font-bold text-slate-800 dark:text-slate-100 mb-1">{job.title}</h3>
                <p className="text-sm text-slate-400 font-medium mb-6">{job.company}</p>
                <div className="flex items-center justify-between">
                   <p className="font-sans font-bold text-slate-800 dark:text-slate-200">{job.salary}</p>
                   <button className="text-primary font-bold text-sm tracking-tight flex items-center gap-1 group">
...
function MunicipalityScreen({ onBack }: { onBack: () => void }) {
  const sections = [
    { title: "Holdings Tax", desc: "Pay property and holding taxes online", icon: Landmark, cat: "Taxes", color: "text-blue-500", bg: "bg-blue-50" },
    { title: "E-Certificates", desc: "Birth, Trade & Character certificates", icon: FileText, cat: "Certificates", color: "text-emerald-500", bg: "bg-emerald-50" },
    { title: "Waste Management", desc: "Garbage collection & area cleaning", icon: Trash2, cat: "Utility", color: "text-amber-500", bg: "bg-amber-50" },
    { title: "Citizen Charter", desc: "Know your rights and city standards", icon: ShieldCheck, cat: "Charter", color: "text-rose-500", bg: "bg-rose-50" },
  ];

  return (
    <div className="flex flex-col pb-32">
       <ScreenHeader title="Municipality" subtitle="Digital Civic Services Portal" onBack={onBack} action={<Building2 size={28} className="text-primary" />} />

       <div className="px-6 mt-4">
          <div className="bg-primary p-8 rounded-[40px] text-white shadow-xl shadow-primary/20 relative overflow-hidden mb-8">
            <div className="absolute top-[-20%] right-[-10%] w-40 h-40 bg-white/10 rounded-full blur-3xl" />
            <h2 className="text-xl font-display font-bold mb-1">Mayor's Online Desk</h2>
            <p className="text-white/70 text-xs font-sans mb-6">Submit complaints directly to authority</p>
            <button className="h-12 bg-white text-primary rounded-2xl px-6 font-display font-black text-xs uppercase tracking-wider flex items-center gap-2">
               Compose Message <Send size={14} />
            </button>
          </div>

          <div className="grid grid-cols-1 gap-4">
            {sections.map((item, i) => (
              <div key={i} className="glass-card dark:bg-slate-900 p-6 rounded-[28px] border-none shadow-lg flex items-center gap-5">
                <div className={`w-14 h-14 ${item.bg} dark:bg-slate-800 rounded-2xl flex items-center justify-center`}>
                  <item.icon size={28} className={item.color} />
                </div>
                <div className="flex-1">
                  <Badge variant="outline" className="mb-1">{item.cat}</Badge>
                  <h3 className="font-display font-bold text-slate-800 dark:text-slate-100">{item.title}</h3>
          ...
          <div className="mt-10">
            <h3 className="text-sm font-black text-slate-400 uppercase tracking-widest mb-6 ml-2">Administrative Info</h3>
            <div className="glass-card dark:bg-slate-900 rounded-[32px] p-6 shadow-xl space-y-5">
               <div className="flex items-center justify-between">
                  <span className="text-xs font-bold text-slate-400">Main Office</span>
                  <span className="text-xs font-black text-slate-800 dark:text-slate-200">Kheyaghat, Mongla</span>
               </div>
               <div className="flex items-center justify-between">
                  <span className="text-xs font-bold text-slate-400">Office Time</span>
                  <span className="text-xs font-black text-slate-800 dark:text-slate-200">09:00 AM - 04:00 PM</span>
               </div>
               <div className="flex items-center justify-between">
                  <span className="text-xs font-bold text-slate-400">Holiday</span>
                  <span className="text-xs font-black text-rose-500">Fri & Saturday</span>
               </div>
            </div>
          </div>
       </div>
